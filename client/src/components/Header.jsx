  ...
  return (
    <header className="flex justify-between items-center bg-primary py-4 px-10">
      ...
      <div className="flex-center relative">
        {!isSearchVisible ? (
          <div
            className="mx-2 p-2 rounded-full hover:bg-secondary hover:text-white transition-all duration-300 ease-in-out shadow-sm cursor-pointer max-lg:hidden"
            onClick={handleSearchClick}
          >
            <FaSearch className=" max-md:w-4 max-md:h-4" />
          </div>
        ) : (
          <motion.form
            initial={{ width: 0, opacity: 0 }}
            animate={{ scale: 1, x: -150, width: "200px", opacity: 1 }}
        ...
        {currentUser ? (
          <Link to="/profile">
            <img
              ...
              alt="profile"
              className="rounded-full ml-7 w-12 h-12 object-cover"
            />
          </Link>
        ) : (
          <div
            className="ml-16  bg-white p-4 rounded-full hover:bg-secondary  transition-all duration-300 ease-in-out shadow-sm "
            onMouseEnter={() => setIsDropdownVisible(true)}
            onMouseLeave={() => setIsDropdownVisible(false)}
          >
            <FaRegUser className="max-md:w-4 max-md:h-4 hover:text-white  cursor-pointer" />
            {isDropdownVisible && (
